...
#define PORT "4444"
#define addr "172.30.2.33"

#define LEFT_KNEE_HEADER "left_knee"
#define RIGHT_KNEE_HEADER "right_knee"
#define LEFT_ELBOW_HEADER "left_elbow"
#define RIGHT_ELBOW_HEADER "right_elbow"
#define BACK_HEADER "back"
...
public:
	...
	void getBodyPartOfFirstPerson(double* xybuf, int bodyPart, const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		const auto& poseKeypoints = datumsPtr->at(0).poseKeypoints;
		xybuf[0] = poseKeypoints[{0, bodyPart, 0}];
		xybuf[1] = poseKeypoints[{0, bodyPart, 1}];
	}
	...
	enum BodyPart {
		...
		LEFT_KNEE = 12,
		LEFT_ANKLE = 13,
		NUM_BODY_PARTS_OF_INTEREST = 13
	};

	int LEFT_BACK[3] = { NECK, LEFT_HIP, LEFT_KNEE };
	int RIGHT_BACK[3] = { NECK, RIGHT_HIP, RIGHT_KNEE };
	int LEFT_KNEE_ANGLE[3] = { LEFT_HIP, LEFT_KNEE, LEFT_ANKLE };
	int RIGHT_KNEE_ANGLE[3] = { RIGHT_HIP, RIGHT_KNEE, RIGHT_ANKLE };
	int RIGHT_ELBOW_ANGLE[3] = { RIGHT_SHOULDER, RIGHT_ELBOW, RIGHT_WRIST };
	int LEFT_ELBOW_ANGLE[3] = { LEFT_SHOULDER, LEFT_ELBOW, LEFT_WRIST };
	int BACK_STRAIGHT_LEFT[3] = { NECK, LEFT_HIP, LEFT_ANKLE };
	int BACK_STRAIGHT_RIGHT[3] = { NECK, RIGHT_HIP, RIGHT_ANKLE };
	int CALF_STRAIGHT_LEFT[2] = { LEFT_KNEE, LEFT_ANKLE };
	int CALF_STRAIGHT_RIGHT[2] = { RIGHT_KNEE, RIGHT_ANKLE };
	
	int bodyAngleAvailable(int* locations, int numLocations, const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		const auto& poseKeypoints = datumsPtr->at(0).poseKeypoints;
		for (int i = 0; i < numLocations; ++i) {
			double x = poseKeypoints[{0, locations[i], 0}];
			double y = poseKeypoints[{0, locations[i], 1}];
	...
	void createBodyVector(double* bodyPartOne, double* bodyPartTwo, double *vector) {
		vector[0] = bodyPartTwo[0] - bodyPartOne[0];
		vector[1] = bodyPartTwo[1] - bodyPartOne[1];
	...
	double getAngle(double *vectorOne, double *vectorTwo) {
		double dotProduct = (vectorOne[0] * vectorTwo[0]) + (vectorOne[1] * vectorTwo[1]);
		double magProduct = vectorNorm(vectorOne) * vectorNorm(vectorTwo);
		if (magProduct == 0) {
			return 0;
		}
		double ratio = dotProduct / magProduct;
		return acos(ratio);
	}

	double getBackAngle(int whichHip, int whichKnee, const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		double neck[2], hip[2], knee[2];
		getBodyPartOfFirstPerson(neck, NECK, datumsPtr);
		getBodyPartOfFirstPerson(hip, whichHip, datumsPtr);
		getBodyPartOfFirstPerson(knee, whichKnee, datumsPtr);

		double backVector[2], thighVector[2];
		createBodyVector(neck, hip, backVector);
		createBodyVector(hip, knee, thighVector);

		return getAngle(backVector, thighVector);
	}

	double getLeftBackAngle(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		return getBackAngle(LEFT_HIP, LEFT_KNEE, datumsPtr);
	}

	double getRightBackAngle(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		return getBackAngle(RIGHT_HIP, RIGHT_KNEE, datumsPtr);
	}

	double getLeftKneeAngle(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		double hip[2], knee[2], ankle[2];
		getBodyPartOfFirstPerson(hip, LEFT_HIP, datumsPtr);
		getBodyPartOfFirstPerson(knee, LEFT_KNEE, datumsPtr);
		getBodyPartOfFirstPerson(ankle, LEFT_ANKLE, datumsPtr);

		double thighVector[2], calfVector[2];
		createBodyVector(hip, knee, thighVector);
		createBodyVector(knee, ankle, calfVector);

		return getAngle(thighVector, calfVector);
	}

	double getRightKneeAngle(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		double hip[2], knee[2], ankle[2];
		getBodyPartOfFirstPerson(hip, RIGHT_HIP, datumsPtr);
		getBodyPartOfFirstPerson(knee, RIGHT_KNEE, datumsPtr);
		getBodyPartOfFirstPerson(ankle, RIGHT_ANKLE, datumsPtr);
	...
	double getRightArmAngle(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		double shoulder[2], elbow[2], wrist[2];
		getBodyPartOfFirstPerson(shoulder, RIGHT_SHOULDER, datumsPtr);
		getBodyPartOfFirstPerson(elbow, RIGHT_ELBOW, datumsPtr);
		getBodyPartOfFirstPerson(wrist, RIGHT_WRIST, datumsPtr);

		double bicepVector[2], forearmVector[2];
		createBodyVector(shoulder, elbow, bicepVector);
		createBodyVector(elbow, wrist, forearmVector);

		return getAngle(bicepVector, forearmVector);
	}

	double getLeftArmAngle(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		double shoulder[2], elbow[2], wrist[2];
		getBodyPartOfFirstPerson(shoulder, LEFT_SHOULDER, datumsPtr);
		getBodyPartOfFirstPerson(elbow, LEFT_ELBOW, datumsPtr);
		getBodyPartOfFirstPerson(wrist, LEFT_WRIST, datumsPtr);
	...
	double getBackLength(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr, int side) {
		double neck[2], hip[2];
		getBodyPartOfFirstPerson(neck, NECK, datumsPtr);
		getBodyPartOfFirstPerson(hip, side, datumsPtr);

		double backVector[2];
		backVector[0] = neck[0] - hip[0];
		backVector[1] = neck[1] - hip[1];

		return vectorNorm(backVector);
	}

	double getLeftBackLength(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		return getBackLength(datumsPtr, LEFT_HIP);
	}

	double getRightBackLength(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		return getBackLength(datumsPtr, RIGHT_HIP);
	}

	double getAnkleLength(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr, int kneeSide, int ankleSide) {
		double knee[2], ankle[2];
		getBodyPartOfFirstPerson(knee, kneeSide, datumsPtr);
		getBodyPartOfFirstPerson(ankle, ankleSide, datumsPtr);

		double calfVector[2];
		calfVector[0] = knee[0] - ankle[0];
		calfVector[1] = knee[1] - ankle[1];

		return vectorNorm(calfVector);
	}

	double getRightAnkleLength(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		return getAnkleLength(datumsPtr, RIGHT_KNEE, RIGHT_ANKLE);
	}

	double getLeftAnkleLength(const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		return getAnkleLength(datumsPtr, LEFT_KNEE, LEFT_ANKLE);
	}

	double getBackAnkleRatio(int useLeft, const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		if (useLeft) {
			return getLeftAnkleLength(datumsPtr) / getLeftBackLength(datumsPtr);
		}
		else {
			return getRightAnkleLength(datumsPtr) / getRightBackLength(datumsPtr);
		}
	}

	double generateBackError(int useLeft, const std::shared_ptr<std::vector<UserDatum>>& datumsPtr) {
		int hipSide = useLeft ? LEFT_HIP : RIGHT_HIP;
		int ankleSide = useLeft ? LEFT_ANKLE : RIGHT_ANKLE;

		double neck[2], hip[2], ankle[2];
		getBodyPartOfFirstPerson(neck, NECK, datumsPtr);
		getBodyPartOfFirstPerson(hip, hipSide, datumsPtr);
		getBodyPartOfFirstPerson(ankle, ankleSide, datumsPtr);

		// Now get distance from ankle to the line connect neck and ankle

		// First, get vector from ankle to neck
		double ankleNeckVector[2];
		ankleNeckVector[0] = neck[0] - ankle[0];
		ankleNeckVector[1] = neck[1] - ankle[1];

		// Next, get vector from back to ankle
		double hipNeckVector[2];
		hipNeckVector[0] = neck[0] - hip[0];
		hipNeckVector[1] = neck[1] - hip[1];

		// Now, project hipNeck onto ankleNeck
		double dot = (ankleNeckVector[0] * hipNeckVector[0]) + (ankleNeckVector[1] * hipNeckVector[1]);
		double normAnkleNeckSquare = vectorNorm(ankleNeckVector) * vectorNorm(ankleNeckVector);

		double projected[2];
		projected[0] = (ankleNeckVector[0] * dot) / normAnkleNeckSquare;
		projected[1] = (ankleNeckVector[1] * dot) / normAnkleNeckSquare;

		// Now subtract the projection from the hipNeckVector

		double distanceVector[2];
		distanceVector[0] = hipNeckVector[0] - projected[0];
		distanceVector[1] = hipNeckVector[1] - projected[1];

		// The back error is the vector norm
		return vectorNorm(distanceVector) / vectorNorm(hipNeckVector);
	}
	...
	// get back calibration vector 

};

void sendData(SOCKET ConnectSocket, char head, double data) {
	// a -> left knee
	// b -> right knee
	// c -> left elbow
	// d -> right elbow
	...
	char charData = (char)((int)(data * 0.708));
	char buffer[2];
	buffer[0] = head;
	buffer[1] = charData;
	std::cout << "sending data: " << (int)charData << std::endl;
	send(ConnectSocket, buffer, 2, 0);
	...
	for (int i = 1; i < 4; i++) {
		buffer[4 - i] = intData % 10;
		intData = intData / 10;
	}
	send(ConnectSocket, buffer, 4, 0);
	*/
	// send(ConnectSocket, (char *)&data, sizeof(data), 0);
}
...
while (!userWantsToExit)
{
	...
	if (opWrapper.waitAndPop(datumProcessed))
	{
		...
		if (datumProcessed->at(0).poseKeypoints.getSize(0)) {
			if (calibrating) {
			// Do calibration here
				if (userOutputClass.bodyAngleAvailable(userOutputClass.BACK_STRAIGHT_LEFT, 3, datumProcessed)) {
					double backErrorLeft = userOutputClass.generateBackError(1, datumProcessed);
					backError = backErrorLeft;
				}
				if (userOutputClass.bodyAngleAvailable(userOutputClass.BACK_STRAIGHT_RIGHT, 3, datumProcessed)) {
					double backErrorRight = userOutputClass.generateBackError(0, datumProcessed);
					if (backError < backErrorRight) {
						backError = backErrorRight;
					}
				}
				if (backError) {
					// std::cout << "back error: " << backError << "\n";
					char errorBuffer[2];
					errorBuffer[0] = 'e';
					errorBuffer[1] = (char)((int)(backError * 255.0));
					send(ConnectSocket, errorBuffer, 2, 0);
				}
			}
			if (userOutputClass.bodyAngleAvailable(userOutputClass.LEFT_KNEE_ANGLE, 3, datumProcessed)) {
				// head -> a
				double leftKneeAngle = (180 * userOutputClass.getLeftKneeAngle(datumProcessed) / 3.14);
				sendData(ConnectSocket, 'a', leftKneeAngle);
				// std::cout << "left knee angle: " << leftKneeAngle << "degrees \n";
			}
			if (userOutputClass.bodyAngleAvailable(userOutputClass.RIGHT_KNEE_ANGLE, 3, datumProcessed)) {
				double rightKneeAngle = (180 * userOutputClass.getRightKneeAngle(datumProcessed) / 3.14);
				// head -> b
				sendData(ConnectSocket, 'b', rightKneeAngle);
				// std::cout << "right knee angle: " << rightKneeAngle << "degrees \n";
			}
			if (userOutputClass.bodyAngleAvailable(userOutputClass.LEFT_ELBOW_ANGLE, 3, datumProcessed)) {
				double leftElbowAngle = (180 * userOutputClass.getLeftArmAngle(datumProcessed) / 3.14);
				// head -> c
				sendData(ConnectSocket, 'c', leftElbowAngle);
				// if (leftElbowAngle >= 90) std::cout << "left arm angle: " << leftElbowAngle << "degrees \n";
			}
			if (userOutputClass.bodyAngleAvailable(userOutputClass.RIGHT_ELBOW_ANGLE, 3, datumProcessed)) {
				double rightElbowAngle = (180 * userOutputClass.getRightArmAngle(datumProcessed) / 3.14);
				// head -> c
				sendData(ConnectSocket, 'd', rightElbowAngle);
				// if (rightElbowAngle >= 90) std::cout << "right arm angle: " << rightElbowAngle << "degrees \n";
			}
